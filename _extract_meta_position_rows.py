...
s = Path('/home/adama/civitaiuk/_source_meta_camp_sex_position.html').read_text(encoding='utf-8', errors='ignore')
start = s.find('<h2 class="wp-block-heading">セックス体位の基本プロンプト 6選</h2>')
if start == -1:
    # fallback: first relevant h2 containing 体位 and 基本
    m = re.search(r'<h2 class="wp-block-heading">[^<]*体位[^<]*基本[^<]*</h2>', s)
    start = m.start() if m else -1
if start == -1:
    raise SystemExit('start not found')
s = s[start:]
end_mark = '<h2 class="wp-block-heading">セックス体位と組み合わせたいプロンプト（関連記事）</h2>'
end = s.find(end_mark)
if end != -1:
    s = s[:end]

# capture H2/H3 and rows with code in first td
...
Path('/home/adama/civitaiuk/_meta_camp_position_rows.tsv').write_text('\n'.join('\t'.join(r) for r in rows), encoding='utf-8')
print('rows', len(rows))
for r in rows[:40]:
    print(r)
